            ...
            <form class="card-body w-100 w-xl-700px px-9" id="kt_create_account_form">
                <div data-kt-stepper-element="content" class="current">
                    <div class="row mb-5">
                        ...
                        <div v-if="store.auditReport.rpt_assessment_control.length" class="card-body p-2">
                            <div class="row mb-2">
                            ...
                            <div class="row pb-5">
                                <div class="row">
                                    ...
                                    <div class="col-md-12 cursor-none">
                                        ...
                                        <input v-model="store.auditReport.rpt_assessment_control[currentControl].compliance_status" class="form-check-input me-2" type="radio" name="account_plan" :value="0">
                                        <span class="fs-6 fw-bold text-muted me-5">Complained</span>
                                        <input v-model="store.auditReport.rpt_assessment_control[currentControl].compliance_status" class="form-check-input me-2" type="radio" name="account_plan" :value="1">
                                        <span class="fs-6 fw-bold text-muted me-5">Non-Complained</span>
                                        <input v-model="store.auditReport.rpt_assessment_control[currentControl].compliance_status" class="form-check-input me-2" type="radio" name="account_plan" :value="2">
                            ...
                            <div class="row mb-10">
                                <div class="col-md-12 mb-20">
                                    ...
                                    <QuillEditor theme="snow" :toolbar="toolbar" v-model:content="store.auditReport.rpt_assessment_control[currentControl].analysis_remarks" content-type="html" ref="quill"                                 
...
const next = async () =>{
    if(store.auditReport.rpt_assessment_control.length){
        await API.post("api/audit/chng-ass-control",store.auditReport.rpt_assessment_control).then((res)=>{
            if(res.status == 200){
                report.value.rpt_assessors = store.auditReport.rpt_assessment_control
                localStorage.setItem('report',JSON.stringify(report.value))
                router.push({path: '/report-summary'})
            }else{
                useToast(res)
            }
        })
    }else{
        useToast("Controls required")
    }
    
}
const back = () =>{
    localStorage.setItem('stepper',5)
